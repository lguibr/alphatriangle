...
from ..utils.types import Experience, StatsCollectorData
...
class LoopHelpers:
    ...
    def calculate_and_log_rates(self):
        ...
        current_buffer_size = float(loop_state["buffer_size"])
        ...
        buffer_size_step = int(current_buffer_size)
        ...
        if self.stats_collector_actor:
            ...
            if steps_delta > 0:
                rate_stats["Rate/Steps_Per_Sec"] = (steps_per_sec, global_step)
                log_msg_steps = f"Steps/s={steps_per_sec:.2f}"

            try:
                self.stats_collector_actor.log_batch.remote(rate_stats)  # type: ignore
                logger.debug(
                    f"Logged rates/buffer at step {global_step} / buffer {buffer_size_step}: "
    ...
    def log_progress_eta(self):
        ...
        if (
            ...
            dq = self.latest_stats_data["Rate/Steps_Per_Sec"]
            if dq:
                steps_per_sec = dq[-1][1]
    ...
    def log_training_results_async(
        ...
        if self.stats_collector_actor:
            stats_batch = {
                ...
                "LearningRate": (current_lr, global_step),
                "Progress/Train_Step_Percent": (train_step_perc, global_step),
                "Progress/Total_Simulations": (total_simulations, global_step),
            }
            if per_beta is not None:
                stats_batch["PER/Beta"] = (per_beta, global_step)
            try:
    ...
    def log_weight_update_event(self, global_step: int):
        ...
        if self.stats_collector_actor:
            try:
                ...
                update_metric = {WEIGHT_UPDATE_METRIC_KEY: (1.0, global_step)}
